...
import { ButtonAgendamento, ButtonNewPokemon, ButtonPaga, Div, Divider, FlexCol, FlexR, FlexRowPaga, FlexTime, Form, ImageIcon, Input, Label, MensagemDeErro, Option, P12Black, P12Cinza, P14Terciary, P14TerciaryAlign, P16Mais, P20, P24, P8, Select, SuccesfullyCard } from "../../styles/consultaPagina";
...
export default function Consulta() {
  ...
  const { formState: { errors, isSubmitSuccessful }, watch, register, reset, handleSubmit, setValue } = contexto;
    ...
    <>
      ...
      <Div>
        {hidden ? (
          ...
          <SuccesfullyCard>
            <P20>
              {isSubmitSuccessful ? "Consulta Agendada" : "Houve um problema no agendamento"}
            </P20>
            <ImageIcon src={isSubmitSuccessful ? "/check.svg" : "/warning.svg"}></ImageIcon>
            <P14TerciaryAlign>
              {isSubmitSuccessful ? `Seu agendamento para dia ${watch("dataAtendimento")}, às ${watch("horarioAtendimento")}, para ${quantidadePokemons} ${quantidadePokemons === 1 ? "pokemon" : "pokemons"} foi realizado com sucesso!` : errors.root?.message}
            </P14TerciaryAlign>
            <ButtonAgendamento onClick={() => { setHidden(true); reset(); }}>Fazer Novo Agendamento</ButtonAgendamento>
          </SuccesfullyCard>
        )}
      </Div>
    </>
